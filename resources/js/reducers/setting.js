...
const initState = {
    ...
    selectedGym:
        localStorage && JSON.parse(localStorage.getItem(SELECTED_GYM_KEY))
            ? JSON.parse(localStorage.getItem(SELECTED_GYM_KEY))
            : {},
    loading: false,
    errorMsg: '',
    successMsg: ''
...
const setting = (state = initState, action = NON_ACTION) => {
    switch (action.type) {
        ...
        case ActionTypes.CHANGE_PWD_FAIL:
            return Object.assign({}, state, {
                errorMsg: 'Update password failed',
                loading: false
            })
